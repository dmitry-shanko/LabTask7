...
public class DatabaseServiceImpl implements DatabaseService
{
	...
	public boolean fillInDatabase() 
	{
		...
		try 
		{
			...
			List<Country> countries = generateCountries(10000);
			this.addCountries(countries);
			List<City> cities = generateCities(10000);
			this.fillCountryAndCities(countries, cities);
			this.addCities(cities);	
			
			countries = null;
			
			List<Address> addresses = generateAddresses(10000);
			this.fillCityAndAddresses(cities, addresses);
			this.addAddresses(addresses);
			
			cities = null;
			
			List<Company> companies = generateCompanies(10000);
			this.addCompanies(companies);
			List<Office> officies = generateOffices(10000);
			...
			List<Position> positions = generatePositions(10000);
			this.addPositions(positions);
			List<Workstation> workstations = generateWorkstations(10000);
